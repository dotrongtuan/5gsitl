...
CONFIG_NAME="${1:-${OMNETPP_CONFIG_NAME:-RuntimeView}}"
...
nohup bash -lc "cd '${PROJECT_ROOT}/omnetpp' && set +u && source '${OMNETPP_ROOT}/setenv' && set -u && BIN=\$(find out -type f \( -name 'omnetpp_sitl' -o -name 'omnetpp_sitl.exe' \) | head -n 1) && \"\${BIN}\" -u Qtenv -c '${CONFIG_NAME}' -n ned ${OMNETPP_INI:-simulations/omnetpp.ini}" \
...
log "OMNeT++ GUI launched with config ${CONFIG_NAME}."
